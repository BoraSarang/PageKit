...
### 주요 변경 [docs]
- 랜딩(`landing/index.html`): 실제 화면 스크린샷 섹션(`#shots`)과 네비 "미리보기" 링크 제거

## v1.0.6 (2026-08-23) — 문서·랜딩 대대적 개편 (기능 최신화)

### 주요 변경 [docs]
- **README 최신화**: 기능 표에 유튜브 형식 확인·복사 행 추가, 스트림 병합 설명을 검토 화면(해상도·예상 용량) 흐름으로 정정, 빠른 시작·로드맵 갱신
- **랜딩 페이지 개편** (`landing/index.html`): hero 카피·브라우저 모형 최신화(품질 점수·유튜브 카드), 문제/솔루션 3번째 카드 교체, 기능 카드 6→8개(유튜브·품질 진단 추가), 스크린샷 플레이스홀더 섹션 신설, 사용법·CTA·푸터(버전 표시) 갱신

## v1.0.5 (2026-08-23) — 리사이즈 루프 버그 수정 + 툴바 팝업 버튼 설명

### 주요 변경 [chrome]
- **다운로드 창 세로 리사이즈 버그 수정**: 다운로드 진행 중 발생하는 고빈도 진행 텍스트 mutation(세그먼트·MB·퍼센트 갱신)이 창 크기 재조정을 계속 자극해 세로가 점점 늘어나던 양성 피드백 루프 차단
  - 다운로드 시작~완료 동안 리사이즈 비활성화(진행 화면은 고정 높이), 완료 시에만 1회 재조정
- **툴바 팝업 버튼 설명 추가**: [사이드 패널에서 분석]·[품질 진단] 버튼 하단에 용도 안내 문구(한 줄) 추가

## v1.0.4 (2026-08-23) — 스트림 카드 정리 (유튜브 한 줄 + 일반 화질 통합 카드)

### 주요 변경 [chrome]
- 유튜브 카드: 형식 상세 줄 제거 → **형식 select + [주소 복사] 한 줄** (select는 정보용, 선택해도 주소 불변)
- 일반 m3u8 마스터 스트림: 흩어진 화질 변형을 **하나의 카드**로 통합 표시 (화질 N개 통합 · 다운로드 가능 + 정보용 형식 select)
