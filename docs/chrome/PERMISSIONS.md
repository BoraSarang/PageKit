...
**버전**: v1.0.12 / **플랫폼**: chrome / **갱신**: 2026-09-17 (v1.0.12, Safari 이식 무관 — chrome 권한 변경 없음)
...
| `declarativeNetRequest` | HLS/DASH 스트림 URL 감지 + 스트림 CDN referer 규칙(병렬 수신 시 원본 Referer/Origin 재생) + 페이지 폴백 모바일 UA 규칙 | 선택 (옵션 ON 시) |
...
- **깨진 링크 실측(v1.0.11)**: 사이드패널에서 [내부 링크 확인] 클릭 시에만 background `fetch` HEAD 요청으로 동일 페이지의 **내부 링크 상태**를 판정 (동시성 5·10s 타임아웃, 4xx/5xx만 broken) — 외부 사이트 요청은 하지 않음
...
| 2026-08-31 | `declarativeNetRequest` 모바일 UA 규칙 추가(옵션 fallbackMobileUA, 페이지 폴백 전용) | v1.0.10 |