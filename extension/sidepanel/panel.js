...
async function analyze(force = false) {
  ...
  try {
    ...
    if (resp?.ok) {
      ...
      mergeStreamVariants().then(() => render()).catch(() => {}) ;
      DebugLogger.feature('PANEL', '분석 표시됨', analysis.stats) ;
      chrome.storage.session.set({ lastAnalysis: { tabId: tab.id, url: tab.url || '', result: resp.data } }).catch(() => {}) ;
      const a = analysis.article || {} ;
      if (!a.found) {
        toast(a.fallback ? '본문 구조를 감지해 일부 항목만 본문으로 표시합니다.' : '본문을 찾지 못해 본문/외부 구분 없이 표시합니다.') ;
      }
    } else {
...
async function mergeStreamVariants() {
  ...
  const found = await Promise.all(m3u8.map(async (it) => {
    try {
      const m = parseM3U8(await fetchStreamText(it.url, 6000), it.url) ;
      return m.isMaster && m.variants.length ? { id: it.id, urls: new Set(m.variants.map((v) => v.url)) } : null ;
    } catch { return null ; }
  })) ;
...
function render() {
  ...
  $('pk-cat-select').innerHTML = cat.map(([v, label, n]) =>
    `<option value="${v}" ${(!isQuick && currentTab === v) ? 'selected' : ''}>${label} (${n})</option>`).join('') ;
...
async function doEnsureThumbs() {
  ...
  for (const el of els) {
    ...
    if (data) { el.style.backgroundImage = `url("${data}")` ; applied += 1 ; }
  }
  ...
  const need = els
    .filter((el) => {
      ...
      return u && u.startsWith('http') && !_thumbCache.has(u) && !el.style.backgroundImage.startsWith('url("data:') ;
    })
    .map((el) => el.dataset.url) ;
  if (!need.length) return ;
  DebugLogger.info('[PANEL] 썸네일 검사 시작', { total: need.length, tabId: analysisSource.tabId, applied }) ;
  const failed = [] ;
  await Promise.all(need.map((u) => new Promise((res) => {
    const img = new Image() ;
    const t = setTimeout(() => { failed.push(u) ; res() ; }, 8000) ;
    img.onload = () => { clearTimeout(t) ; res() ; } ;
    img.onerror = () => { clearTimeout(t) ; failed.push(u) ; res() ; } ;
    img.src = u ;
  }))) ;
  ...
  for (let i = 0; i < failed.length; i += 10) {
    ...
    const resp = await chrome.runtime.sendMessage({ type: MSG.THUMB_FETCH, payload: { tabId: analysisSource.tabId, urls: batch } }) ;
    const map = resp?.ok ? resp.data : {} ;
    for (const u of batch) if (map[u]) { _thumbCache.set(u, map[u]) ; ok += 1 ; }
  }
...
function showAdblockHint(failedCount) {
  ...
  DebugLogger.warn('[PANEL] 광고 차단 의심: 썸네일 요청 전 경로 실패 — 브라우저 필터 확인 안내', { failed: failedCount }) ;
...
$('pk-copy-links').addEventListener('click', async () => {
  ...
  if (await copyText(text)) {
    ...
    DebugLogger.error('[PANEL] 링크 복사 실패', 'Clipboard API/execCommand 모두 실패', { code: 'E-CHR-UI-1002' }) ;
