...
export async function ensureReferer(host, referer) {
  const existing = refererRules.get(host);
...
function broadcast() {
  ...
  chrome.action
    .setBadgeText({
      text: running.filter((j) => j.state === 'active').length
        ? String(running.filter((j) => j.state === 'active').length)
        : '',
    })
    .catch(() => {});
}
...
async function zipPack(items, tabId) {
  ...
  for (let i = 0; i < usable.length; i++) {
    ...
    BGLogger.info(
      'DLZIP',
      `수신 ${i + 1}/${usable.length} ${name} (${(data.byteLength / 1024).toFixed(0)}KB)`
    );
  }
  ...
  BGLogger.feature(
    'DLZIP',
    `ZIP 저장 완료 ${(blob.size / 1048576).toFixed(1)}MB (${usable.length}건) → ${filename}`
  );
...
async function runDownload(job, viaPage = false) {
  ...
  try {
    ...
    const needsExt =
      extOf(job.item.url) &&
      !/\.(png|jpe?g|gif|webp|avif|mp4|webm|mp3|m4a|zip|pdf|m3u8|mpd)$/i.test(name);
    ...
    if (job.item.referer && !viaPage) {
      try {
        await ensureReferer(domainFrom(job.item.url), job.item.referer);
      } catch (e) {
        BGLogger.warn('DL', `Referer 규칙 등록 실패 ${e.message}`);
      }
    }
    let downloadId;
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message?.type === 'pk.dl.start') {
      ...
      if (!items.length) {
        sendResponse({
          ok: false,
          error: { code: 'E-CHR-DL-1001', message: '선택된 항목이 없습니다.' },
        });
        return false;
      }
      ...
      if (zipMode) {
        zipPack(items, tabId)
          ...
          .catch((e) =>
            sendResponse({
              ok: false,
              error: { code: 'E-CHR-DL-1002', message: `ZIP 패키징 실패: ${e.message}` },
            })
          );
        return true;
      }
      const settings = storage.getSettings().then((s) => {
        ...
        Promise.all(Array.from({ length: workers }, run)).then(() =>
          sendResponse({ ok: true, data: { started: items.length } })
        );
      });
      return true;
    }
    ...
    if (message?.type === 'pk.thumb.fetch') {
      ...
      BGLogger.info(
        'THUMB',
        `썸네일 폴백 요청 tab=${tabId} n=${urls.length} first=${urls[0]?.split('/').pop()?.slice(0, 24)}`
      );
      ...
      const fetchFunc = async (urls) => {
        ...
        const toData = (bmp) => {
          const w = 120,
            h = Math.max(1, Math.round((bmp.height * w) / bmp.width));
      ...
      const runFetch = (world, list) =>
        chrome.scripting
          .executeScript({ target: { tabId, frameIds: [0] }, world, func: fetchFunc, args: [list] })
          .then(([res]) => res?.result || []);
      (async () => {
        ...
        if (failed.length) {
          BGLogger.info(
            'THUMB',
            `MAIN 실패분 확장 오리진(BG) 재시도 n=${failed.length} first=${failed[0]?.url?.split('/').pop()?.slice(0, 24)}`
          );
            ...
            for (const u of chunk) {
              try {
                ...
                if (!r.ok || !bmp) {
                  BGLogger.warn(
                    'THUMB',
                    `BG 재시도 실패 ${u.slice(0, 110)} → ${r.status} ${r.headers.get('content-type')} ${b.size}b`
                  );
                ...
                BGLogger.warn(
                  'THUMB',
                  `BG 재시도 예외 ${u.slice(0, 110)} → ${String(e).slice(0, 70)}`
                );
        ...
        BGLogger.info(
          'THUMB',
          `썸네일 폴백 완료 ok=${Object.keys(map).length}/${urls.length} err=${JSON.stringify(errs)}`
        );
