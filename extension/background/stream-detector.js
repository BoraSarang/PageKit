...
const PROGRESSIVE_ITAGS = new Set([
  5, 6, 17, 18, 22, 34, 35, 36, 37, 38, 43, 44, 45, 46, 82, 83, 84, 85,
]);
  ...
  storage
    ...
    .then((list) => {
      ...
      const cap = {
        ...
        format: PROGRESSIVE_ITAGS.has(itag)
          ? 'progressive'
          : AUDIO_ITAGS.has(itag)
            ? 'audio-only'
            : itag
              ? 'video-only'
              : 'unknown',
      ...
      BGLogger.debug(
        'STREAM',
        `유튜브 캡처 itag=${itag} (${cap.label || '라벨 없음'} · ${cap.format}) ${list.length}개`
      );
    })
    .catch(() => {});
};
...
async function applyRules(enabled) {
  try {
    ...
    if (enabled && !webRequestRegistered) {
      chrome.webRequest.onBeforeRequest.addListener(onGooglevideo, {
        urls: ['*://*.googlevideo.com/*'],
      });
      chrome.webRequest.onHeadersReceived.addListener(onGooglevideoResponse, {
        urls: ['*://*.googlevideo.com/*'],
      });
    ...
    BGLogger.info(
      'STREAM',
      enabled ? '스트림 감지 규칙 활성화 (m3u8 + 유튜브 캡처)' : '스트림 감지 규칙 비활성화'
    );
  } catch (e) {
...
export function initStreamDetector() {
  chrome.runtime.onMessage.addListener((message, _sender) => {
    if (
      message?.type === 'pk.settings.set' &&
      message.payload &&
      'streamDetect' in message.payload
    ) {
