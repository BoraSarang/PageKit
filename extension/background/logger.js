...
const BGLogger = {
  debug: (tag, msg, meta) =>
    DebugLogger.debug(`[${tag}] ${msg}`, meta ? `meta=${JSON.stringify(meta)}` : ''),
  info: (tag, msg, meta) =>
    DebugLogger.info(`[${tag}] ${msg}`, meta ? `meta=${JSON.stringify(meta)}` : ''),
  warn: (tag, msg, meta) =>
    DebugLogger.warn(`[${tag}] ${msg}`, meta ? `meta=${JSON.stringify(meta)}` : ''),
  error: (tag, msg, meta) =>
    DebugLogger.error(`[${tag}] ${msg}`, meta ? `meta=${JSON.stringify(meta)}` : ''),
  perf: (tag, msg, meta) => DebugLogger.perf(`[${tag}] ${msg}`, meta?.ms ?? 0),
  cache: (tag, msg, meta) => DebugLogger.cache(`[${tag}] ${msg}`, meta?.ms ?? 0),
  feature: (tag, msg, meta) =>
    DebugLogger.feature(`[${tag}] ${msg}`, meta ? `meta=${JSON.stringify(meta)}` : ''),
};
