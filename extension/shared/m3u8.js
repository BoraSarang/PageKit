...
function isoDurationToSeconds(value) {
  const m = String(value || '').match(/^P(?:(\d+(?:\.\d+)?)Y)?(?:(\d+(?:\.\d+)?)M)?(?:(\d+(?:\.\d+)?)D)?(?:T(?:(\d+(?:\.\d+)?)H)?(?:(\d+(?:\.\d+)?)M)?(?:(\d+(?:\.\d+)?)S)?)?$/i) ;
  if (!m) return null ;
  const Y = parseFloat(m[1] || '0'), Mo = parseFloat(m[2] || '0'), D = parseFloat(m[3] || '0') ;
  const H = parseFloat(m[4] || '0'), Mi = parseFloat(m[5] || '0'), S = parseFloat(m[6] || '0') ;
...
export function parseMPD(text, baseUrl) {
  ...
  const mediaPresentationDuration = isoDurationToSeconds(root.getAttribute('mediaPresentationDuration')) ;
  const period = root.querySelector('Period') ;
  if (!period) return { isDash: true, segs: [], initUrl: null } ;
  // 비디오 적응 세트 우선, 없으면 오디오
  let as = period.querySelector('AdaptationSet[contentType="video"], AdaptationSet[contentType="audio"]') ;
  ...
  const rep = reps.reduce((a, b) => ((parseInt(b.getAttribute('bandwidth') || '0', 10)) > (parseInt(a.getAttribute('bandwidth') || '0', 10)) ? b : a), reps[0]) ;
  const repBase = new URL(rep.querySelector('BaseURL')?.textContent || '.', new URL(as.querySelector('BaseURL')?.textContent || '.', baseUrl).href).href ;
  const meta = {
    isDash: true, segs: [], initUrl: null, type: 'mp4',
    totalSeconds: mediaPresentationDuration, codecs: rep.getAttribute('codecs') || '',
    width: rep.getAttribute('width') || '', height: rep.getAttribute('height') || '',
  } ;
  const resolve = (u) => { try { return new URL(u, repBase).href } catch { return null } } ;
  // 1) SegmentList: Initialization + SegmentURL 나열
  const segList = rep.querySelector('SegmentList') || as.querySelector('SegmentList') || period.querySelector('SegmentList') ;
  if (segList) {
  ...
  const tmpl = rep.querySelector('SegmentTemplate') || as.querySelector('SegmentTemplate') || period.querySelector('SegmentTemplate') ;
  if (tmpl) {
    const init = tmpl.getAttribute('initialization') ;
    if (init) meta.initUrl = resolve(init.replace(/\$RepresentationID\$/g, rep.getAttribute('id') || '0').replace(/\$Bandwidth\$/g, rep.getAttribute('bandwidth') || '0')) ;
    ...
    if (mediaTmpl) {
      ...
      if (duration > 0 && mediaPresentationDuration > 0) {
        ...
        const name = (i) => {
          ...
          u = u.replace(/\$Number\$/g, String(startNumber + i - 1)).replace(/\$Number%(\d+)d\$/g, (_, w) => String(startNumber + i - 1).padStart(Number(w), '0')) ;
  ...
  const segBase = rep.querySelector('SegmentBase') || as.querySelector('SegmentBase') || period.querySelector('SegmentBase') ;
  if (segBase) {
