...
(() => {
  ...
  function clearHighlights() {
    document.querySelectorAll('.pk-highlight-node').forEach((el) => el.classList.remove('pk-highlight-node')) ;
  ...
  function findArticleRoot() {
    ...
    for (const el of document.querySelectorAll('article, main, [role="main"], .article, .post, .entry-content, .blog-post, #content, #main')) {
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    if (!guide) { guide = document.createElement('div'); guide.className = 'pk-highlight-guide'; document.documentElement.appendChild(guide) ; }
    const x = Math.min(dragState.x1, dragState.x2), y = Math.min(dragState.y1, dragState.y2) ;
