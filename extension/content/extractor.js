...
(() => {
  ...
  function extractArticle() {
    const ReadabilityCtor =
      globalThis.Readability || (globalThis.Readability && globalThis.Readability.default);
  ...
  function collectArticleUrls(articleHtml) {
    ...
    for (const v of container.querySelectorAll(
      'video source[src], video[src], audio source[src], audio[src], iframe[src]'
    )) {
    ...
    const add = (el, url, source) => {
      ...
      try {
        ...
        if (
          ...
          imageTypeOf(url) === 'unknown' &&
          u.pathname.startsWith(location.pathname)
        )
          return;
      } catch {
        /* 잘못된 URL 무시 */
  ...
  function collectVideos() {
    ...
    const push = (el, url, kind) => {
      ...
      videos.push({
        ...
        type: lower.endsWith('.m3u8')
          ? 'hls'
          : lower.endsWith('.mpd')
            ? 'dash'
            : (url.split('?')[0].split('.').pop() || 'mp4').toLowerCase(),
        label: '',
        inArticle: false,
    ...
    try {
      ...
      const rx =
        /(?:file|source|src)\s*[:=]\s*["'](https?:\/\/[^"']+?\.(?:mp4|m3u8|mpd)(?:\?[^"']*)?)["']/gi;
    ...
    try {
      ...
      const MEDIA_HOST =
        /\/(?:[^/]+\.)*(v\d+-webapp-?prime|v\d+-webapp|tiktokcdn|googlevideo|cdn-video)\./i;
      const NON_MEDIA =
        /\.(js|css|png|jpe?g|svg|gif|webp|ico|json|woff2?|wasm|html?|vtt|srt|sbv|ttml|dfxp|ass|sub|xml)(\?|$)/i;
      const SUBTITLE_PATH = /\/(timedtext|subtitle|caption)[/?]/i;
      for (const e of performance.getEntriesByType('resource')) {
        const n = e.name;
        if (/\.(mp4|m3u8|mpd|ts|webm|mov)(\?|$)/i.test(n)) {
          playerUrls.add(n);
          continue;
        }
      ...
      for (const s of a.querySelectorAll('source[src]')) {
        ...
        if (u?.startsWith('http') && !seen.has(u)) {
          ...
          audios.push({
            ...
            type: 'audio',
            inArticle: false,
            downloadable: downloadableOf(u, 'audio'),
          });
        }
      }
    }
  ...
  function collectLinks() {
    ...
    const seen = new Set();
    const FILE_RE =
      /\.(pdf|zip|rar|7z|tar|gz|mp4|webm|mkv|mp3|wav|flac|xlsx?|docx?|pptx?|csv|json|xml)(\?|#|$)/i;
  ...
  function collectStreams() {
    ...
    const candidates = [
      ...document.querySelectorAll(
        'video[src*=".m3u8"], video source[src*=".m3u8"], video[src*=".mpd"], video source[src*=".mpd"]'
      ),
      ...document.querySelectorAll(
  ...
  function collectBgImages(articleContainer, extraScan) {
    ...
    if (extraScan) {
      for (const el of document.querySelectorAll(
        'ytd-thumbnail, [class*="thumb"], [class*="image"], [class*="img"], [class*="photo"], [id*="thumb"]'
      ))
        pushFrom(el);
    }
    return out;
  ...
  function analyze() {
    ...
    if (!article || articleMedia.length < 10) {
      const sel =
        'article, main, [role="main"], .post-content, .entry-content, .article-body, .product-info, .goods-detail, .detail-area, .xans-product-detail, .xans-product-image, .product-image, .prod-atf, [class*="product-image"], [class*="prod-image"], #content, .content';
    ...
    const fallbackInfo = articleFallback
      ? {
          tag: articleFallback.tagName,
          cls: String(articleFallback.className || '').slice(0, 50),
          imgs: articleFallback.querySelectorAll('img').length,
        }
      : null;
    const result = {
      url: location.href,
      ...
      article: article
        ? {
            ...
            bodyTextLen: article.textContent?.length || 0,
          }
    ...
    DebugLogger.info(
      `[EXTRACT] 분석 완료 (${perf.toFixed(1)}ms, img=${images.length} vid=${videos.length} links=${links.length} article=${Boolean(article)})`
    );
    ...
    return new Promise((resolve) => {
      ...
      const send = () => {
        for (const f of frames) {
          ...
          try {
            f.contentWindow.postMessage({ type: 'pk.frame.analyze', requestId }, '*');
          } catch {
            pending.delete(f);
          }
        }
      ...
      const finish = () => {
          ...
          for (const key of ['videos', 'audios', 'streams']) {
            for (const it of media[key]) {
              ...
              if ((key === 'videos' || key === 'audios') && /\.(m3u8|mpd)(\?|#|$)/i.test(it.url)) {
                if (!it.protocol)
                  it.protocol = it.url.toLowerCase().endsWith('.mpd') ? 'dash' : 'hls';
        ...
        DebugLogger.feature(
          'EXTRACT',
          `iframe 병합 완료 (${received.size}/${frames.length}, 시도 ${attempt + 1}회)`,
          result.stats
        );
        resolve();
      };
  ...
  async function collectNaverVod(result) {
    try {
      ...
      const player = document.querySelector(
        '.webplayer-internal-video, video[poster*="tvcast"], video[poster*="pstatic"]'
      );
      if (!player) return;
      // vodplay 메타 요청은 페이지 로드 후 ~2초에 플레이어가 자동 호출 — 아직 없으면 잠시 대기 후 재확인
      let req = performance
        .getEntriesByType('resource')
        .find((e) => e.name.includes('neonplayer/vodplay/'));
      ...
      const urls = [
        ...new Set(text.match(/https?:[^"'\s\\]+?\.(?:m3u8|mpd)(?:\?[^"'\s\\]*)?/gi) || []),
      ];
  ...
  async function mergeCapturedStreams(result) {
    try {
      ...
      for (const c of caps) {
        ...
        if (c.itag) {
          const pIdx = result.media.streams.findIndex(
            (s) => s.itag === c.itag && s.source === 'youtube-player'
          );
          if (pIdx >= 0) {
          ...
          const prev = result.media.streams.findIndex(
            (s) => s.source === 'youtube-capture' && s.itag === c.itag
          );
          if (prev >= 0) {
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    ...
    if (message?.type === 'pk.fetch.stream') {
      ...
      if (window !== window.top) return false;
      const u = message.payload?.url,
        rng = message.payload?.range ?? null;
        ...
        try {
          ...
          if (!res.ok && (res.status === 401 || res.status === 403)) {
            ...
            const same = live.filter(
              (n) => new URL(n).searchParams.get('itag')?.toString() === itag
            );
    ...
    if (message?.type === 'pk.analyze.page') {
      ...
      function waitPageStable(maxMs = 12000) {
        return new Promise((resolve) => {
          ...
          const iv = setInterval(() => {
            ...
            if (imgs.length === lastImgs && frames.length === lastFrames && pending === 0)
              stable += 1;
            else stable = 0;
            lastImgs = imgs.length;
      ...
      (async () => {
        try {
          ...
          DebugLogger.error('[EXTRACT] 분석 실패', `${e.name}: ${e.message}`, {
            code: 'E-CHR-NET-1001',
          });
          sendResponse({
            ok: false,
            error: { code: 'E-CHR-NET-1001', message: '페이지 분석에 실패했습니다.' },
          });
        }
      })();
