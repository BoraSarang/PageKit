...
ws.on('message', (data) => {
  ...
  if (msg.id === 1) {
    if (msg.result && msg.result.exceptionDetails) {
      console.log('EXCEPTION:', JSON.stringify(msg.result.exceptionDetails.exception?.description || msg.result.exceptionDetails.text));
    } else if (msg.result && msg.result.result) {
      const r = msg.result.result;
      if (r.type === 'undefined') console.log('RESULT: undefined');
      else if (r.type === 'object' && r.subtype === 'error') console.log('ERROR:', r.description || r.value);
